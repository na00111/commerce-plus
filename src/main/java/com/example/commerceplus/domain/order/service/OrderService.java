...
public class OrderService {
    ...
    // 주문 생성
    @Transactional
    public CreateOrderResponse createOrder(Member member, CreateOrderRequest requestDto) {

        // 요청받은 상품 ID 목록 추출 (비어있으면 장바구니 전체 사용). requestDto에서 추출
        List<Long> productIds = extractProductIds(requestDto);
        if (productIds == null || productIds.isEmpty()) {
            throw new /* 커스텀 예외 클래스 */(ErrorCode.INVALID_INPUT);
        }

        // 해당 상품들을 DB에서 조회, productRepository로 조회
        List<Product> products = productRepository.findAllById(productIds);
        if (products.isEmpty()) {
            throw new /* 커스텀 예외 클래스 */(ErrorCode.PRODUCT_NOT_FOUND);
        }
    ...
    private void validateStock(List<Product> products) {
        for (Product product : products) {
            if (product.getStock() < 1) { // 재고 부족 조건
                throw new /* 커스텀 예외 클래스 */ (ErrorCode.INSUFFICIENT_STOCK);
            }
        }
    }
