...
조회 이후 실제 취소 처리 시점에 주문 상태를 다시 확인하여
이미 결제가 완료된 주문은 처리 대상에서 제외합니다.

재고 복구 시에도 상품 ID 순으로 정렬한 뒤 비관적 락을 획득하여
...
이미 존재하는 상품을 다시 추가하면 새로운 행을 만들지 않고 기존 수량에 합산합니다.
...
주문번호는 서버에서 UUID를 기반으로 생성하며
DB PK와 독립된 고유 식별값으로 사용합니다.
...
PortOne 실결제에서는 해당 값을 외부 결제와 내부 Payment를 연결하는 식별자로 사용합니다.
...
주문과 결제 상태를 분리하여
결제 실패, 주문 취소, PortOne 보상 취소 실패 등
각 비즈니스 상황을 구분하여 저장할 수 있도록 구성했습니다.
